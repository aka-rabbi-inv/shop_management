...
from shop_management_mulytic.settings import BASE_DIR
...
def order_product(request):
    ...
    if request.method == "POST":
        ...
        for content in data['product_detail']:
            try:
                ...
                if content['product_code'] not in products:
                    products[content['product_code']] = quantity
                else:
                    products[content['product_code']] += quantity
                ...
                return JsonResponse({"error": f"No product with name {content['product_name']}."}, status=406)
            except OutofStockError:

                return JsonResponse({"error": f"Out of stock. Currently have {product.current_stock} units of {product.product_name}."}, status=406)
        for code,quant in products.items():
            ...
            product.save()
            CustomerOrder.objects.create(products=product, name=data['name'], phone=data['phone'], email=data['email'], quantity=quant, row_status=1)
